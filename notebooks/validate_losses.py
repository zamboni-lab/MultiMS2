...
with app.setup:
    ...
    class Settings:
        ...
        output: str | None = field(
            ...
            metadata={"help": "Path to write validated spectra MGF"},
        )
        invalid_out: str | None = field(
            default="scratch/invalidated_spectra.mgf",
            metadata={"help": "Path to write invalidated spectra MGF"},
        )
        stats: bool = field(
            default=True,
            metadata={"help": "Print summary statistics when run as script"},
        )
...
class MGFLossValidator:
    ...
    def parse_adduct(self, adduct_str: str) -> Dict:
        ...
        if self.loss_reactions:
            ...
            loss_pattern = (
                r"-(\d*)("
                + "|".join(re.escape(k) for k in self.loss_reactions)
                + r")(?=$|[+\-\]])"
            )
    ...
    def can_form_adduct(self, smiles: str, adduct: str) -> Tuple[bool, str]:
        ...
        for loss in sorted(set(loss_list)):
            ...
            if rxn is not None:
                ...
                depth_guard = 0
                while (
                    frontier and enumerated < requested and depth_guard < requested * 4
                ):
                    new_frontier = []
                    for tpl in frontier:
                        try:
